  ...
  try {
    ...
    firebase.auth().onAuthStateChanged(async function (user) {
      ...
      try {
        ...
        console.warn(
          "⚠️ Não foi possível criar sessão Appwrite:",
          error.message
        );
      }
  ...
  try {
    ...
    promise.then(
      function (response) {
        ...
        const viewResult = appwriteStorage.getFileView(
...
function loadConfig(config) {
  ...
  if (config.logoImageUrl) {
    document.querySelector(
      "#logoPreview"
    ).style.backgroundImage = `url(${config.logoImageUrl})`;
  }
...
function update() {
  ...
  if (footerNome)
    footerNome.textContent = document.getElementById("empresaNome").value;

  const bannerH1 = document.getElementById("bannerH1");
  if (bannerH1)
    bannerH1.textContent = document.getElementById("bannerTitulo").value;

  const bannerP = document.getElementById("bannerP");
  if (bannerP)
    bannerP.textContent = document.getElementById("bannerSubtitulo").value;
  ...
  if (sobreTextoPreview)
    sobreTextoPreview.textContent = document.getElementById("sobreTexto").value;

  const sobreImagemPreview = document.getElementById("sobreImagemPreview");
  if (sobreImagemPreview)
    sobreImagemPreview.style.backgroundImage = `url(${
      document.getElementById("sobreImagem").value
    })`;

  const telPreview = document.getElementById("telPreview");
  if (telPreview)
    telPreview.textContent = document.getElementById("telefone").value;

  const telPreview2 = document.getElementById("telPreview2");
  if (telPreview2)
    telPreview2.textContent = document.getElementById("telefone2").value;

  const emailPreview = document.getElementById("emailPreview");
  if (emailPreview)
    emailPreview.textContent = document.getElementById("email").value;

  const enderecoPreview = document.getElementById("enderecoPreview");
  if (enderecoPreview)
    enderecoPreview.textContent = document.getElementById("endereco").value;
...
function renderProdutos() {
  ...
  // PRECISA DE EXPANSÃO COM DETALHES
  if (list)
    list.innerHTML = state.produtos
      .map(
        (p, i) => `
    <div class="produto-item ${expandedProductId === i ? "expanded" : ""}" 
         id="produto-item-${i}">
        <div class="produto-header" onclick="toggleProductExpanded(${i})">
            <strong>${p.nome}</strong>
            <span>${expandedProductId === i ? "▼" : "▶"}</span>
        </div>
        ${
          expandedProductId === i
            ? `
            <div class="produto-expanded">
