...
async function initializeApp(user) {
  ...
  try {
    if (typeof firebase === "undefined") {
      console.error(
        "❌ [app.js] Firebase não está disponível em initializeApp."
      );
      ...
      async () => {
        try {
          if (!window.appwriteClient) {
            console.error(
              "❌ [app.js] Cliente Appwrite não encontrado após appwriteReady."
            );
            return;
          }
          ...
          console.log(
            "✅ [app.js] Sessão anônima Appwrite criada com sucesso."
          );
        } catch (error) {
          // Erro 409 (Conflict) geralmente significa que uma sessão já existe, o que é seguro ignorar.
          if (error.code !== 409) {
            console.warn(
              "⚠️ [app.js] Não foi possível criar sessão anônima no Appwrite:",
              error.message
            );
          } else {
...
async function handleImageUpload(event, targetInputId, previewSelector) {
 ...
 try {
    // ✅ IMPORTANTE: Validar arquivo ANTES
    ...
    // ✅ NOVO: Criar sessão anônima se não existir
    try {
        if (window.appwriteClient) {
            const account = new Appwrite.Account(window.appwriteClient);
            ...
            try {
                ...
                // Se erro 409, sessão já existe (tudo bem!)
                if (sessionError.code !== 409) {
                    console.warn('⚠️ Erro ao criar sessão:', sessionError.message);
                }
            }
        }
    } catch (authError) {
        console.warn('⚠️ Erro de autenticação Appwrite:', authError);
    }

    // ✅ FAZER O UPLOAD
    const promise = window.appwriteStorage.createFile(
        '691287dd003bc7260e15', // Bucket ID
        'unique()',
        compressedFile
    );
    ...
    promise.then(function (response) {
        ...
        // NOVO: Se for imagem de produto, atualiza o focuser
        ...
        // Atualiza redes sociais se necessário
  ...
  try {
    ...
    try {
      if (socialLinksDoc && socialLinksDoc.exists) {
        ...
        console.log(
          "📱 Dados de redes sociais carregados:",
          JSON.stringify(socialData, null, 2)
        );

        if (socialData && Array.isArray(socialData.links)) {
          console.log(
            "✅ socialData.links é um array com",
            socialData.links.length,
            "itens."
          );
          ...
          if (config.socialLinks.length > 0) {
            console.log(
              "✅ Redes sociais configuradas e válidas:",
              config.socialLinks
            );
          } else {
            console.warn(
              "⚠️ Nenhum link social válido encontrado após o filtro."
            );
          }
        } else {
          console.warn(
            "⚠️ Dados de redes sociais inválidos ou socialData.links não é um array:",
            socialData
          );
          config.socialLinks = [];
        }
        ...
        console.log(
          "ℹ️ Nenhum documento de redes sociais (social_links/data) encontrado."
        );
        config.socialLinks = [];
      }
    } catch (error) {
    ...
    state.socialLinks = Array.isArray(config.socialLinks)
      ? [...config.socialLinks]
      : [];
    loadConfig(config);
...
async function saveConfig() {
  try {
    ...
    const globalSettingsRef = clientDocRef
    ...
    if (error.code === "permission-denied") {
      ...
      console.error(
        "📋 Verifique suas regras de segurança no Firebase Console"
      );
...
function loadConfig(config) {
  ...
  if (!logoTypeInput) {
    console.warn(
      "Elemento 'logoType' não encontrado. O painel pode estar incompleto."
    );
  }
  ...
  if (document.getElementById("bannerTitulo"))
    document.getElementById("bannerTitulo").value = config.bannerTitulo || "";
  if (document.getElementById("bannerSubtitulo"))
  ...
  if (document.getElementById("logoImageUrl"))
  ...
  if (document.getElementById("faviconImageUrl"))
    document.getElementById("faviconImageUrl").value =
      config.faviconImageUrl || "";
  ...
  if (config.contato) {
    ...
    if (latDisplay)
      latDisplay.textContent = (config.contato.latitude || -23.5505).toFixed(4);
    if (lonDisplay)
      lonDisplay.textContent = (config.contato.longitude || -46.6333).toFixed(
        4
      );
...
function update() {
  ...
  if (!iframe) {
    console.error("❌ Iframe de preview não encontrado!");
    return;
  }
  const iframeDoc = iframe.contentDocument || iframe.contentWindow.document;
  if (!iframeDoc) {
    console.error("❌ Documento do iframe não pôde ser acessado.");
    return;
  }

  const fontUrl = document.getElementById("fontUrl");
  if (fontUrl && fontUrl.value) {
    let fontLink = iframeDoc.getElementById("dynamic-font");
    if (!fontLink) {
  ...
  if (fontFamilyInput && fontFamilyInput.value) {
    const previewSite = iframeDoc.querySelector("body"); // Target body for font
    if (previewSite) previewSite.style.fontFamily = fontFamilyInput.value;
  }

  const logoContainer = iframeDoc.getElementById("logo");
  const adminLogoIcon = document.getElementById("adminLogoIcon"); // This is in the main doc
  const logoTypeInput = document.getElementById("logoType");
  ...
  const empresaNome = empresaNomeInput
  ...
  // ATUALIZADO: Atualizar Logo no Cabeçalho do Admin (This stays in the main document)
  if (adminLogoIcon) {
  ...
  const footerNome = iframeDoc.getElementById("footerNome");
  if (footerNome) footerNome.textContent = empresaNome;

  const bannerH1 = iframeDoc.getElementById("bannerH1");
  const bannerTituloInput = document.getElementById("bannerTitulo");
  if (bannerH1 && bannerTituloInput)
    bannerH1.textContent = bannerTituloInput.value;

  const bannerP = iframeDoc.getElementById("bannerP");
  const bannerSubtituloInput = document.getElementById("bannerSubtitulo");
  if (bannerP && bannerSubtituloInput)
    bannerP.textContent = bannerSubtituloInput.value;

  const banner = iframeDoc.querySelector(".banner");
  const bannerImagemInput = document.getElementById("bannerImagem");
  if (banner && bannerImagemInput)
    banner.style.backgroundImage = `url(${bannerImagemInput.value})`;
  ...
  // These displays are in the main doc
  ...
  // This preview is in the main doc
  ...
  iframeDoc
    .querySelectorAll(".site-nav-links a")
    .forEach((a) => (a.style.color = corPrimaria));
  iframeDoc
    .querySelectorAll(".contact-icon")
    .forEach((icon) => (icon.style.background = corPrimaria));

  const ctaBtn = iframeDoc.querySelector(".cta-btn");
  if (ctaBtn) ctaBtn.style.background = corSecundaria;

  const sobreTextoPreview = iframeDoc.getElementById("sobreTextoPreview");
  const sobreTextoInput = document.getElementById("sobreTexto");
  if (sobreTextoPreview && sobreTextoInput)
    sobreTextoPreview.textContent = sobreTextoInput.value;

  const sobreImagemPreview = iframeDoc.getElementById("sobreImagemPreview");
  const sobreImagemInput = document.getElementById("sobreImagem");
  if (sobreImagemPreview && sobreImagemInput)
    sobreImagemPreview.style.backgroundImage = `url(${sobreImagemInput.value})`;

  const telPreview = iframeDoc.getElementById("telPreview");
  const telInput = document.getElementById("telefone");
  if (telPreview && telInput) telPreview.textContent = telInput.value;

  const telPreview2 = iframeDoc.getElementById("telPreview2");
  const telInput2 = document.getElementById("telefone2");
  if (telPreview2 && telInput2) telPreview2.textContent = telInput2.value;

  const emailPreview = iframeDoc.getElementById("emailPreview");
  const emailInput = document.getElementById("email");
  if (emailPreview && emailInput) emailPreview.textContent = emailInput.value;

  const enderecoPreview = iframeDoc.getElementById("enderecoPreview");
  const enderecoInput = document.getElementById("endereco");
  if (enderecoPreview && enderecoInput)
    enderecoPreview.textContent = enderecoInput.value;

  if (state.modules) {
    const sobreSection = iframeDoc.querySelector(".sobre-section");
    if (sobreSection)
      sobreSection.classList.toggle("hidden", !state.modules.sobre);

    const navSobre = iframeDoc.querySelector(".nav-sobre");
    if (navSobre) navSobre.classList.toggle("hidden", !state.modules.sobre);

    const produtosSection = iframeDoc.querySelector(".produtos-section");
    if (produtosSection)
      produtosSection.classList.toggle("hidden", !state.modules.produtos);

    const navProdutos = iframeDoc.querySelector(".nav-produtos");
    if (navProdutos)
      navProdutos.classList.toggle("hidden", !state.modules.produtos);

    const contatoSection = iframeDoc.querySelector(".contato-section");
    if (contatoSection)
      contatoSection.classList.toggle("hidden", !state.modules.contato);

    const navContato = iframeDoc.querySelector(".nav-contato");
    if (navContato)
      navContato.classList.toggle("hidden", !state.modules.contato);
  }
...
function renderProdutos() {
  ...
  const grid = iframeDoc ? iframeDoc.getElementById("produtosGrid") : null;
  if (grid) {
      ...
      .map(
        (p) => `
            <div class="product-card">
                ...
                )}); background-position: ${escapeHtml(
          p.foco || "center"
        )};"></div>
  ...
  // This list is in the main document
...
function initImageFocuser(initialFoco = "50% 50%") {
  ...
  function updateFoco(x, y) {
    ...
    let newX = Math.max(
      targetSize / 2,
      Math.min(x, containerWidth - targetSize / 2)
    );
    let newY = Math.max(
      targetSize / 2,
      Math.min(y, containerHeight - targetSize / 2)
    );
    ...
    const focusX = Math.round(
      ((newX - targetSize / 2) / (containerWidth - targetSize)) * 100
    );
    const focusY = Math.round(
      ((newY - targetSize / 2) / (containerHeight - targetSize)) * 100
    );
  ...
  const [initialX, initialY] = initialFoco
    .split(" ")
  ...
  const initialX_px =
    initialX * (initialContainerRect.width - targetSize) + targetSize / 2;
  const initialY_px =
    initialY * (initialContainerRect.height - targetSize) + targetSize / 2;
...
function renderSocialLinks() {
  ...
  // Renderiza no footer do site público (no iframe)
  if (iframeDoc) {
      const socialIconsFooter = iframeDoc.querySelector(".social-icons-footer");
      if (socialIconsFooter) {
        socialIconsFooter.innerHTML = state.socialLinks
          .map(
            (link) => `
                <a href="${escapeHtml(
                  link.url
                    ...
                    ${
                      link.icon ||
                      `<i class="${getIconClass(link.name || link.nome)}"></i>` ||
                      "📱"
                    }
                </a>
            `
        ...
        (link, i) => `
            <div class="social-link-item">
                <div class="social-preview">
                    ...
                    <span class="social-name">${escapeHtml(
                      link.name || link.nome || ""
                    )}</span>
                </div>
...
function openSocialModal() {
  ...
  if (predefinedTab) {
    predefinedTab.innerHTML = redesPredefinidas
      .map(
        (rede) => `
...
function saveSocialLink() {
  ...
  if (!url.startsWith("http://") && !url.startsWith("https://")) {
    showToast(
      "⚠️ URL inválida. Use o formato completo (http:// ou https://)",
      "warning"
    );
    return;
  }
